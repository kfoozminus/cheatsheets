...
- `deploymentspec.revisionHistoryLimit` number of old replicasets to allow rollback - defaults to 10
```
spec:
...
- new revision is created if pod template is changed
- added a bad image, then scaled up. deployment created new pods from old replicaset too (and bad image pods too, maxSurge matched = total bad/good/old/new pods `CURRENT`. QJenny maxUnavailable is not maxUnready/maxInvalid, huh?)
